...
def register(validated_data):
    ...
    if register:
        response.add_data(register).add_message('User registered successfully').add_status_code(200)
        return jsonify(response.build()), response.status_code
    else:
        response.add_message('User already exists').add_status_code(400).add_data()
        return jsonify(response.build()), response.status_code
...
def login(validated_data):
    ...
    if login:
        response.add_data(login).add_message('User logged in successfully').add_status_code(200)
        return jsonify(response.build()), response.status_code
    else:
        response.add_message('Invalid credentials').add_status_code(400).add_data()
        return jsonify(response.build()), response.status_code
